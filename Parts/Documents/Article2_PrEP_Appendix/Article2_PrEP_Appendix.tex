...
\usepackage[a4paper,hmargin=0.5cm,vmargin=1.2cm]{geometry}
...
\right)
\end{equation}
is called \textit{mixing matrix}, and its elements verify $0 < \rho_{ij} < 1$ and $\sum_j \rho_{ij} =1$. Since we assumed that individuals mix preferentially within the same risk group, we have $\rho_{ii} > 1/2$. 
...
The rate at which $i$-risk susceptibles acquire HIV by forming sexual partnerships with infected individuals is called the \textit{force of infection} \cite{Jacquez1988} and is given by
\begin{equation}
...
%\[
%1- \frac{\mathcal{I}^{\rm ES}(\hat{p}(\varepsilon,r),r)}{\mathcal{I}^{\rm ES}(0, 0)}.
%\]
...
\begin{minipage}{\textwidth}
	...
	\caption[Definition, initial value and calibrated values for the model parameters]{%
	      	\textbf{Definition, initial value and calibrated values for the model parameters}\\
...
\begin{minipage}{\textwidth}
	...
	\caption[ Calibration to the HIV epidemiology for MSM in the Paris region]{%
		\textbf{Calibration to the HIV epidemiology for MSM in the Paris region}\\
...
\begin{figure}[H]
	...
	\textbf{Flow diagram for the compartmental model of HIV transmission among MSM, when PrEP is not available as a prevention method}\\
...
%% Figure S4 - Voluntary PrEP coverage and Incidence reduction for p=0.86
\newpage
\begin{figure}[H]	
\small	
\centering		
...
%%% Figure S? - Incidence reduction
%\newpage
%\begin{figure}[H]
%	\centering	
%	\includegraphics{Figures/Fig_S6.pdf}
%	\caption[Incidence reduction]{%
%		\textbf{Incidence reduction}\\
%	}
%	\label{fig:IncRed_NoChangesIntestingRates}
%\end{figure}
