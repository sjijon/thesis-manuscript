...
%	\includegraphics[width=0.7\textwidth]{DRAFT_FigsAndDocs/FIG}
	%%
	\includegraphics[width=12cm]{Figures/PrEP/HIV/HIVinfection}
...
In most high-income settings, the population of men who have sex with men (MSM) is one of the most affected by the HIV epidemic, which is currently continuing to expand \cite[]{UNAIDS_Data2018,WHO_KeyPop2016,Beyrer2012}. The high transmission of HIV among MSM may be explained by biological factors, such as the high probability of HIV transmission through receptive anal intercourse, as well as by behavioral factors, such as unprotected sexual intercourses, large number of casual partners and using recreational drugs \cite[]{Beyrer2012}.
...
Among on-PrEP MSM in the Paris region, some evidence of risk compensation (drop in condom use) has been observed \cite[]{Molina2018}. Also, a 30-month dropout rate of $\sim 32\%$ \cite[]{Costagliola2019} was observed, which reveals the need of understanding and addressing PrEP persistence.
...
%	\includegraphics[width=0.7\textwidth]{DRAFT_FigsAndDocs/FIG}
	...
	\captionsetup{width=0.85\textwidth}
	\caption[PrEP users in Île-de-France, by semester, by year]{%
		{\bf PrEP users in Île-de-France, by semester, by year}\\
...
To compute the effective reproduction number for the ODE system~{\normalfont \hyperlink{Article2supp.5}{(S6)}}, we follow the methods and notation developed by \cite{VanDenDriessche2002}, where $R(p,\varepsilon)$ is defined as the larger eigenvalue of the \emph{next generation matrix} \cite[]{VanDenDriessche2008,Diekmann1990}. 
...
\right),
\end{equation}
where the first eight compartments correspond to infected individuals. Then, we identify the vectors $\mathscr{F}$ and $\mathscr{V}$ (which represent the new infections and the transfers in and out of the compartments, respectively), satisfying $dX/dt = \mathscr{F} - \mathscr{V}$:
...
\subsubsection*{\it Regarding PrEP effectiveness}
...
\subsubsection*{\it Regarding PrEP coverage}
