...
As of June 2018, only 35 countries had at least one public policy implemented regarding PrEP rollout; most of them (15 countries) in the European region \cite[]{HodgesMameletzis2018}. Global efforts thus remain far from the \cite{WHO_KeyPop2016} recommendations to scale up PrEP programs among high-risk populations. 
...
Cost-effectiveness analyses have shown that a PrEP rollout among MSM at high risk of infection is cost-effective in comparison to other interventions \cite[]{DurandZaleski2018,Revill2017,Cambiano2017,Nichols2016,Ouellet2015,Desai2008}, but may remain costly nevertheless \cite[]{Juusola2013,Gomez2012}. Moreover, cost-effectiveness analyses have shown to be highly sensitive to the price of PrEP \cite[]{Coleman2017}. This exhibits the need of reducing the price of PrEP, which remains a key barrier to broadly provide PrEP through public health programs \cite[]{ECDC_PrEP2016}.
...
On one hand, the MSM community has shown to be highly aware about HIV risk of infection, which has resulted in the practice of risk-reduction strategies \cite[]{McFarland2011,Velter2015}. Paradoxically, these preventive practices may mislead individuals into believing their sexual behaviors are not risky enough \cite[]{Golub2014}, so additional prevention measures, such as PrEP, may no longer be adopted \cite[]{Young2014}. On the other hand, MSM have also shown to be highly aware of PrEP \cite[]{Frankis2016,Grov2016} and PrEP has shown to be well-accepted (if available) among MSM who identify themselves at high risk of infection \cite[]{Frankis2016,Ferrer2016,Taylor2014,Aghaizu2013}. 
...
%	\includegraphics[width=0.7\textwidth]{DRAFT_FigsAndDocs/FIG}
	...
	\captionsetup{width=0.85\textwidth}
	\caption[PrEP users in Île-de-France, by semester and year]{%
		{\bf PrEP users in Île-de-France, by semester and year}\\
...
Modeling studies have evaluated the impact of a PrEP rollout on the HIV epidemic among MSM, predicting a remarkable reduction in the number of new HIV infections \cite[]{Gomez2012,Kim2014,Punyacharoensin2016,Jenness2016,Robineau2017,Rozhnova2018,Palk2018,Rozhnova2019}. In particular, recent mathematical models have studied HIV epidemic elimination through PrEP rollouts among MSM \cite[]{Rozhnova2018,Scott2018,Hansson2020}. In these models, different levels of PrEP coverage are considered \cite[]{Gomez2012,Punyacharoensin2016,Robineau2017,Rozhnova2018}, including full coverage \cite[]{Kim2014}. However, high levels of PrEP coverage, which are required to substantially impact or eliminate HIV epidemics, may not be granted in the real world. Therefore, it remains to know whether and how PrEP coverage levels can be reached and maintained, in the long term. 
...
\label{PrEP:Article}
A scientific article presenting our findings, titled ``Can HIV epidemics among men who have sex with men be eliminated through participation to PrEP rollouts?'' \cite[]{Jijon2021}, was submitted for publication to the \emph{AIDS} journal in January 2021, and is currently under review. 

The preprint of the main text is included below, along with the supplementary material, where the mathematics of our model is presented in detail. The computations and proofs of the analytical results presented in the article's supplementary material are available in~\secref{PrEP:AdditionalMaterial} of this chapter. Additional figures are also found in~\secref{PrEP:AdditionalMaterial}. The implementation of HIV prevention programs are further discussed in~\secref{PrEP:Discussion}, as well as the limitations and perspectives of our modeling choices. 
...
%		height = 25cm,
		...
		pagecommand = {},
		pages=-]
		{Parts/Documents/Article2/Article2_PrEP}
...
\includepdf[pages = -,
		...
		pagecommand = { },
		height = 27cm]
		{Parts/Documents/Article2_Appendix/Article2_PrEP_Appendix}

\section{Additional material} 
\label{PrEP:AdditionalMaterial}

Section~\ref{PrEP:Computations} presents the computations and proofs of the results shown in~\secref{PrEP:ArticlePDF}. Namely, the explicit computations of some epidemiological indicators, using the ODE system (see sections~\hyperlink{sec:Comp_PrEPeffectiveness}{a)} to \hyperlink{sec:Comp_R}{c)}), the thresholds for the PrEP effectiveness and the perceived relative cost yielding epidemic control and elimination (see sections~\hyperlink{sec:Comp_EpiControl}{d)}, \hyperlink{sec:Comp_EpiElim}{e)} and~\hyperlink{sec:Comp_rCrE}{h}), and a description of the numerical approximation of the voluntary PrEP coverage (see~\hyperlink{sec:Comp_VolPrEP}{g)}). 
...
Here, we analyze a reduced version of our HIV transmission model (cf.~\artref{eq.(S6)}{PrEP:ArticlePDF}) adapted to describe the IPERGAY trial \cite[]{Molina2018}, in order to estimate the parameter representing the effectiveness of PrEP,~$\varepsilon$. 
...
\end{aligned}
\end{equation}
we obtain $\varepsilon = 0.86$. Hence, we found the same value as the efficacy estimated in from the ANRS IPERGAY trial, thus corroborating our modeling choices in~\secref{PrEP:ArticlePDF}.
...
The equations for the disease-free state (DFS) of the ODE system~\hyperlink{Article2supp.5}{(S6)} are given by 
...
Epidemic control induced by PrEP uptake is defined as a reduction of the effective reproduction number due to PrEP adoption, i.e., $R(p,\varepsilon)<R(0,\varepsilon)$; see~\secref{PrEP:ArticlePDF}. Here, we study the monotony of the reproduction number $R(p,\varepsilon)$ with respect to the PrEP parameters, PrEP effectiveness, $\varepsilon$, and PrEP coverage, $p$, in order to find the conditions necessary and sufficient to have $R(p,\varepsilon)<R(0,\varepsilon)$.

\subsubsection*{\it Monotony with respect to PrEP effectiveness}
...
\subsubsection*{\it Monotony with respect to PrEP coverage}
...
Note that \eqref{eq:epsilonC} can also be obtained from looking to conditions on $\varepsilon$ for which $\varphi(\varepsilon)\geq0$. Therefore, $\varphi(\varepsilon)$ can be interpreted as the combined effectiveness of PrEP and condoms; wich can be straightforwardly identified in the scenario where individuals do not change testing behavior, i.e., where $\theta=\theta_P$; see~\figref{fig:SA_phi} below.
...
We computed the effective reproduction number in the absence of PrEP, i.e., $R(0,0)$, for the $\sim500$ simulations calibrating our model. A mean value of 2.2 (95\%~CI: 1.7--2.6) resulted from our model calibration, which is broadly in agreement with the traditionally used  results of \cite{Anderson1991}, where $R_0$ has been estimated at 2--5 among MSM.
...
Current guidelines for PrEP prescription require testing for HIV every 3 months \cite[]{Molina2018}. However, frequent HIV screening may be perceived as a barrier for some individuals \cite[]{Calabrese2016} and thus, frequent HIV testing may not be guaranteed in the long run. 

We ran our algorithm considering a scenario where MSM do not follow these recommendations, and keep testing for HIV at the same rate than before adopting PrEP (i.e., every $\theta_P=\theta=3.1$ years). The resulting voluntary coverage for PrEP is shown in~\secref{PrEP:ArticlePDF}. Here, we show additional figures, similar to those included in~\secref{PrEP:ArticlePDF}, such as the function $\varphi (\varepsilon)$, the number of infections among on-PrEP MSM (\figref{fig:SA_InfectionsOnPrEP}), the PrEP-induced reduction in HIV incidence (depicted in~\subfigref{fig:SA_Lambda_rCrE}{A}), the relative costs thresholds (computed using~\algoref{algo:rCrE}; see~\subfigref{fig:SA_Lambda_rCrE}{B}) and the voluntary PrEP coverage resulting from misperception of HIV risk  (\figref{fig:SA_Misperception}), for the scenario where individuals do not change their HIV testing behavior.

We also used $\sim500$ sets of parameters calibrating our model, in order to compute the probability of epidemic elimination (see~\subfigref{fig:SA_ProbElim}{A}) and to obtain confidence intervals for the PrEP effectiveness thresholds for epidemic control, $\varepsilon_{\rm C}$, and elimination, $\varepsilon_{\rm E}$ (see~\subfigref{fig:SA_ProbElim}{B}). \subfigref{fig:SA_ProbElim}{C} depicts the relative cost thresholds for epidemic control and elimination, computed numerically for the $\sim500$ parameters sets. Similarly to \secref{sec:Comp_rCrE}, $r_{\rm E}(\varepsilon)$ is the boundary between region II and regions I--IV together. Therefore, for $\varepsilon < \varepsilon_{\rm E}$, we identified $r_{\rm E}(\varepsilon)$ as the highest value of $r$ for which $\hat{p}(\varepsilon,r)=1$; cf.~\figref{fig:SA_Lambda_rCrE} and \subfigref{fig:SA_ProbElim}{C}.
...
%	\includegraphics[width=0.7\textwidth]{DRAFT_FigsAndDocs/FIG}
	%%
	\includegraphics[width=0.5\textwidth]{Figures/PrEP/Results_SensitivityAnalysis/phi}
	\caption[The combined effectiveness of PrEP and condom use, for the scenario where on-PrEP MSM do not change HIV testing behavior]{%
		{\bf The combined effectiveness of PrEP and condom use}\\
%		, for the scenario where on-PrEP MSM do not change HIV testing behavior}\\
	\raggedright
	The combined effectiveness of PrEP and condom use among on-PrEP high-risk MSM, relative to the effectiveness of condom use alone among off-PrEP high-risk MSM, $\varphi(\varepsilon)$, when MSM do not change their testing behavior (i.e., $\theta_P=\theta=3.1$ years), for a condom effectiveness of 70\% and a PrEP-induced drop in condom use from 30\% to 20\%; see~\secref{PrEP:ArticlePDF}, eq.~(S4). As stated in \secref{sec:Comp_EpiElim}, epidemic control induced by PrEP and condom use is possible if and only if $\varphi(\varepsilon) > 0$; that is, if $\varepsilon \geq \varepsilon_{\rm C}$.}
	\label{fig:SA_phi}
\end{figure}
...
%	\includegraphics[width=0.7\textwidth]{DRAFT_FigsAndDocs/FIG}
	%%
	\includegraphics[width=\textwidth]{Figures/PrEP/Results_SensitivityAnalysis/InfectionsOnPrEP}
\caption[HIV infections despite PrEP uptake, for the scenario where on-PrEP MSM do not change HIV testing behavior]{%
	{\bf HIV infections despite PrEP uptake}\\
	Number of infections among on-PrEP MSM {\bf (A)} as a function of PrEP coverage ($p$) and PrEP effectiveness, and {\bf (B)} for several fixed values of $\varepsilon$. For a given value of $\varepsilon$, the maximum is depicted by a black circle. For low values of PrEP effectiveness, the number of infections among on-PrEP MSM is strictly increasing with PrEP coverage. For high levels of PrEP effectiveness, the behavior of the infections among on-PrEP MSM is similar to that in~\figref{fig:InfectionsOnPrEP}: a threshold for PrEP coverage may be identified, from which the number of new infections among on-PrEP MSM decreases with PrEP coverage. }
	\label{fig:SA_InfectionsOnPrEP}
\end{figure}
...
%	\includegraphics[width=0.9\textwidth]{DRAFT_FigsAndDocs/FIG}
	%%
	\includegraphics[width=0.9\textwidth]{Figures/PrEP/Results_SensitivityAnalysis/LAMBDA_rCrE}
	\caption[The PrEP-induced relative reduction in HIV incidence and the relative cost thresholds for epidemic control and elimination, for the scenario where on-PrEP MSM do not change HIV testing behavior]{%
	{\bf The PrEP-induced relative reduction in HIV incidence and the relative cost thresholds for epidemic control and elimination}\\
	\raggedright
	\textbf{(A)} Color map of the PrEP-induced reduction in the overall endemic HIV incidence rate, as a function of $\varepsilon$ and $r$. PrEP effectiveness thresholds required to reach epidemic control and epidemic elimination are given by $\varepsilon_{\rm C} = 8\%$ and $\varepsilon_{\rm E} = 58\%$, respectively. Four regions can be identified: region~IV, where all individuals adopt PrEP ($\hat{p}=100\%$; cf.~\artref{fig.~3}{PrEP:ArticlePDF}) and $\varepsilon_{\rm C} \leq \varepsilon<\varepsilon_{\rm E}$, so the epidemic is controlled; region~III, where no MSM uses PrEP ($\hat{p}=0\%$) and thus, there is no reduction in HIV incidence; region~II, where some, but not enough MSM use PrEP ($0\% < \hat{p} <100\%$) because $r$ remains high and thus the epidemic is controlled; and region~I (marked by blue stripes), where epidemic elimination is possible. %
	\textbf{(B)} The relative costs thresholds for epidemic control and elimination as functions of PrEP effectiveness (denoted $r_{\rm C}(\varepsilon)$ and $r_{\rm E}(\varepsilon)$, respectively); cf.~fig. 3 in~\secref{PrEP:ArticlePDF}. $r_{\rm C}(\varepsilon)$ is the boundary between regions II and region III, while $r_{\rm E}(\varepsilon)$ is the boundary between region II and regions I--IV together. We identified these thresholds numerically, from the voluntary PrEP coverage, using~\algoref{algo:rCrE}. Both figures were obtained for a typical parameter set calibrating our model (cf.~\artref{table S4}{PrEP:ArticlePDF}), assuming that individuals have a fair perception of HIV risk and do not change their testing behaviors once adopting PrEP (i.e., $\theta_P=\theta=3.1$ years).}
	\label{fig:SA_Lambda_rCrE}
\end{figure}
...
%	\includegraphics[width=0.6\textwidth]{DRAFT_FigsAndDocs/FIG}
	...
	\caption[The probability of HIV elimination and boundary uncertainty for the four-regions structure, for the scenario where on-PrEP MSM do not change HIV testing behavior]{%
		{\bf The probability of HIV elimination and boundary uncertainty for the four-regions structure}\\
	\raggedright
...
%	\includegraphics[width=0.7\textwidth]{DRAFT_FigsAndDocs/FIG}
	%%
	\includegraphics[width=0.5\textwidth]{Figures/PrEP/Results_SensitivityAnalysis/Misperception}
	\caption[The voluntary PrEP coverage, assuming misperception of the HIV infection risk, for the scenario where on-PrEP MSM do not change HIV testing behavior]{%
		{\bf The voluntary PrEP coverage assuming misperception of the HIV infection risk}\\
	\raggedright
	Decision-making based on a misperceived risk of acquiring HIV significantly reduces the size of region~I, where epidemic elimination is possible (blue stripes). The figure was obtained using the same calibrated parameter set as in~\figref{fig:SA_Lambda_rCrE}. The PrEP effectiveness thresholds for epidemic control and epidemic elimination are, respectively, $\varepsilon_{\rm C} = 8\%$ and $\varepsilon_{\rm E} = 58\%$.}
...
We assume that, in the long term, high-risk MSM on PrEP practice high-quality adherence. That is, we assume that PrEP users, take PrEP pills correctly, continuously and for life. This assumption may not represent current reality in the Paris region, where only about 85\% of PrEP users got a prescription renewal the following year, since PrEP rollout \cite[]{ANSM_Truvada2019,ANSM_Truvada2020}. Modeling recurrent decision-making or the drop of PrEP while being sexually active may thus be interesting to study. 

Still, our results may be useful for future presentations of prevention against HIV, which may perceived as less costly by individuals, such as long-lasting injectable PrEP \cite[]{Marshall2018} and vaccination against HIV \cite[]{Burton2018}. 
